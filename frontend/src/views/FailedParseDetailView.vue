            ...
            <div class="grid grid-cols-2 gap-lg bg-surface-container-lowest p-md rounded-xl">
                ...
                <div>
                    ...
                    <p class="font-body-md">
                        <router-link v-if="tx.rule_id" :to="`/parsing-rules/add?id=${tx.rule_id}`">{{ (tx.rule_name && tx.rule_name.trim() && tx.rule_name.trim().toLowerCase() !== 'unnamed rule') ? tx.rule_name.trim() : 'Unnamed Rule' }}</router-link>
                        <span v-else>{{ (tx.rule_name && tx.rule_name.trim() && tx.rule_name.trim().toLowerCase() !== 'unnamed rule') ? tx.rule_name.trim() : 'Unknown / Not recorded' }}</span>
                    </p>
                </div>
            </div>
